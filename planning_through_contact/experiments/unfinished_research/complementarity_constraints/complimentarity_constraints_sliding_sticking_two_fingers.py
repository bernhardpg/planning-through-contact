...
p_BF1_0 = np.array([-0.1 - box.width / 2, 0])
p_BF2_0 = np.array([-0.2 - box.width / 2, box.height / 2 - 0.01])
th_I = 0
...
finger1_phi_0 = -p_BF1_0[0] - box.width / 2
finger2_phi_0 = -p_BF2_0[0] - box.width / 2
...
# Finger variables
finger1_gamma = prog.NewContinuousVariables(N, "finger_gamma_1")
finger1_lambda_n = prog.NewContinuousVariables(N, "finger_lambda_n_1")
finger1_lambda_f_comps = prog.NewContinuousVariables(N, 2, "finger_lambda_f_1")

finger2_gamma = prog.NewContinuousVariables(N, "finger_gamma_2")
finger2_lambda_n = prog.NewContinuousVariables(N, "finger_lambda_n_2")
finger2_lambda_f_comps = prog.NewContinuousVariables(N, 2, "finger_lambda_f_2")

p_BF1 = prog.NewContinuousVariables(N, 2, "p_BF1")
delta_p_BF1 = prog.NewContinuousVariables(N, 2, "delta_p_BF1")
p_BF2 = prog.NewContinuousVariables(N, 2, "p_BF2")
delta_p_BF2 = prog.NewContinuousVariables(N, 2, "delta_p_BF2")
...
finger1_phi = -p_BF1[:, 0] - box.width / 2
finger2_phi = -p_BF2[:, 0] - box.width / 2

finger1_delta_tang_pos = p_BF1[:, 1]  # TODO: Replace this with general jacobian
finger1_delta_tang_pos_comps = np.vstack(
    [-finger1_delta_tang_pos, finger1_delta_tang_pos]
).T  # (N, 2)
finger1_lambda_f = finger1_lambda_f_comps[:, 1] - finger1_lambda_f_comps[:, 0]
f_F1_B = [np.array([n, -f]) for n, f in zip(finger1_lambda_n, finger1_lambda_f)]
f_F1_W = np.vstack([R @ f for R, f in zip(R_WB, f_F1_B)])

finger2_delta_tang_pos = p_BF2[:, 1]  # TODO: Replace this with general jacobian
finger2_delta_tang_pos_comps = np.vstack(
    [-finger2_delta_tang_pos, finger2_delta_tang_pos]
).T  # (N, 2)
finger2_lambda_f = finger2_lambda_f_comps[:, 1] - finger2_lambda_f_comps[:, 0]
f_F2_B = [np.array([n, -f]) for n, f in zip(finger2_lambda_n, finger2_lambda_f)]
f_F2_W = np.vstack([R @ f for R, f in zip(R_WB, f_F2_B)])
...
for i in range(N):
    ...
    lhs = finger1_gamma[i] * e * h + finger1_delta_tang_pos_comps[i]
    rhs = finger1_lambda_f_comps[i]
    sliding_comp_constraints.append(add_complimentarity_constraint(prog, lhs, rhs))

    lhs = mu * finger1_lambda_n[i] - np.sum(finger1_lambda_f_comps[i])
    rhs = finger1_gamma[i]
    sliding_comp_constraints.append(add_complimentarity_constraint(prog, lhs, rhs))

    # Add contact/non-contact complimentarity constraints
    lhs = finger1_phi[i]
    rhs = finger1_lambda_n[i]
    contact_comp_constraints.append(add_complimentarity_constraint(prog, lhs, rhs))

    # Finger 2
    # Add sliding/sticking complimentarity constraints
    lhs = finger2_gamma[i] * e * h + finger2_delta_tang_pos_comps[i]
    rhs = finger2_lambda_f_comps[i]
    sliding_comp_constraints.append(add_complimentarity_constraint(prog, lhs, rhs))

    lhs = mu * finger2_lambda_n[i] - np.sum(finger2_lambda_f_comps[i])
    rhs = finger2_gamma[i]
    sliding_comp_constraints.append(add_complimentarity_constraint(prog, lhs, rhs))

    # Add contact/non-contact complimentarity constraints
    lhs = finger2_phi[i]
    rhs = finger2_lambda_n[i]
    contact_comp_constraints.append(add_complimentarity_constraint(prog, lhs, rhs))
    ...
    c = eq(f_F1_B[i] + f_F2_B[i] + f_cp1_B[i] + f_cp2_B[i] + f_grav_B[i], 0)
    prog.AddLinearConstraint(c)

    # Torque balance
    sum_torques = (
        cross_2d(p_BF1[i], f_F1_B[i])
    ...
    prog.AddLinearConstraint(p_BF1[i, 1] <= box.height / 2)
    prog.AddLinearConstraint(p_BF1[i, 1] >= -box.height / 2)

    prog.AddLinearConstraint(p_BF2[i, 1] <= box.height / 2)
    prog.AddLinearConstraint(p_BF2[i, 1] >= -box.height / 2)

    # Backward euler: x_next = x_curr + h * f(x_curr, u_curr)
    if i == 0:
        prog.AddLinearConstraint(eq(p_WB[i], p_WB_0 + h * delta_p_WB[i]))
        prog.AddLinearConstraint(eq(p_BF1[i], p_BF1_0 + h * delta_p_BF1[i]))
        ...
        prog.AddLinearConstraint(eq(p_BF1[i], p_BF1[i - 1] + h * delta_p_BF1[i]))
...
# th_F = 0.0
end_x_pos = 0.5
# prog.AddLinearConstraint(eq(p_BF1[N - 1], p_BF1_0))
# prog.AddLinearConstraint(eq(p_BF2[N - 1], p_BF2_0))
...
prog.AddQuadraticCost(np.trace(delta_p_BF1.T @ delta_p_BF1))  # type: ignore
prog.AddQuadraticCost(np.trace(delta_p_BF2.T @ delta_p_BF2))  # type: ignore
...
def animate_vals(result):
    ...
    p_BF1_sol = result.GetSolution(p_BF1)
    p_BF2_sol = result.GetSolution(p_BF2)
    p_cp1_W_sol = evaluate_np_expressions_array(p_cp1_W, result)
    p_cp2_W_sol = evaluate_np_expressions_array(p_cp2_W, result)
    f_F1_W_sol = evaluate_np_expressions_array(f_F1_W, result)
    f_F2_W_sol = evaluate_np_expressions_array(f_F2_W, result)
    ...
    p_BF1_sol = np.vstack([p_BF1_0, p_BF1_sol])
    p_BF2_sol = np.vstack([p_BF2_0, p_BF2_sol])
    p_cp1_W_sol = np.vstack([p_cp1_W_0, p_cp1_W_sol])
    p_cp2_W_sol = np.vstack([p_cp2_W_0, p_cp2_W_sol])
    f_F1_W_sol = np.vstack([f_F_W_0, f_F1_W_sol])
    f_F2_W_sol = np.vstack([f_F_W_0, f_F2_W_sol])
    ...
    p_WF1_sol = np.vstack([p + R @ f for p, R, f in zip(p_WB_sol, R_WB_sol, p_BF1_sol)])
    p_WF2_sol = np.vstack([p + R @ f for p, R, f in zip(p_WB_sol, R_WB_sol, p_BF2_sol)])
    ...
    viz_com_points = [
        ...
        VisualizationPoint2d(p_WF1_sol, FINGER_COLOR),
        VisualizationPoint2d(p_WF2_sol, FINGER_COLOR),
    ...
    viz_contact_forces = [
        VisualizationForce2d(p_WF1_sol, CONTACT_COLOR, f_F1_W_sol),
        VisualizationForce2d(p_WF2_sol, CONTACT_COLOR, f_F2_W_sol),
...
def plot_vals(result: MathematicalProgramResult, title: str):
    ...
    p_WB_sol = result.GetSolution(p_WB)
    p_BF_sol = result.GetSolution(p_BF1)
    ...
    finger2_lambda_n_sol = result.GetSolution(finger2_lambda_n)
    finger2_lambda_f_sol = evaluate_np_expressions_array(finger2_lambda_f, result)
    finger2_phi_sol = evaluate_np_expressions_array(
        finger2_phi, result
    )  #  type: ignore
    ...
    axs[0, col].plot(finger1_lambda_n_sol)
    axs[0, col].set_title("finger1_lambda_n")
    axs[0, col].set_xlim(0, N)

    # Plot finger_lambda_f
    axs[1, col].plot(finger1_lambda_f_sol)
    axs[1, col].set_title("finger1_lambda_f")
    axs[1, col].set_xlim(0, N)

    # Plot finger_phi
    axs[2, col].plot(finger1_phi_sol)
    axs[2, col].set_title("finger1_phi")
    axs[2, col].set_xlim(0, N)

    # Plot finger_lambda_n
    axs[3, col].plot(finger2_lambda_n_sol)
    axs[3, col].set_title("finger2_lambda_n")
    axs[3, col].set_xlim(0, N)

    # Plot finger_lambda_f
    axs[4, col].plot(finger2_lambda_f_sol)
    axs[4, col].set_title("finger2_lambda_f")
    axs[4, col].set_xlim(0, N)

    # Plot finger_phi
    axs[5, col].plot(finger2_phi_sol)
    axs[5, col].set_title("finger2_phi")
    axs[5, col].set_xlim(0, N)
