    ...
    def DoCalcOutput(self, context, output):
        ...
        if time_step == 0 or (last_command == [-999] * self._num_positions).all():
            ...
            for i in range(self._num_positions):
                ...
                velocity = abs(
                    (joint_positions_commanded[i] - last_command[i]) / time_step
                )
                if velocity > self._joint_velocity_limits[i]:
                    joint_positions_clamped[i] = (
                        last_command[i]
                        + self._joint_velocity_limits[i] * time_step * sign
                    )
                    velocity_clamped[i] = self._joint_velocity_limits[i] * sign
                else:
                    joint_positions_clamped[i] = joint_positions_commanded[i]
                    velocity_clamped[i] = velocity * sign
