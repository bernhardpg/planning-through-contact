...
class RobotStateToRigidTransform(LeafSystem):
    def __init__(self, plant: MultibodyPlant, robot_model_name: str, offset=None):
        ...
        self._num_positions = self._plant.num_positions(
            self._robot_model_instance_index
        )
        self._num_velocities = self._plant.num_velocities(
            self._robot_model_instance_index
        )
    ...
    def DoCalcOutput(self, context, output):
        ...
        self._plant.SetPositions(
            self._plant_context, self._robot_model_instance_index, q
        )
        pose = self._plant.EvalBodyPoseInWorld(
            self._plant_context, self._plant.GetBodyByName("pusher")
        )
        if self._offset:
