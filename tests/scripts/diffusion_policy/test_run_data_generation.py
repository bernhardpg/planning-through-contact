...
def test_run_data_generation(config_path: str, config_dir: str):
    ...
    command = f'python {PYTHON_SCRIPT} --config-dir {config_dir} --config-name {config_path}'
    os.system(command)
    passed = _passed_test()
...
def _passed_test() -> bool:
    ...
    if not os.path.exists(f"{trajectory_path}/traj_0_0/analysis/rounded_traj_trajectory.pdf"):
        return False
    ...
    zarr_path = 'tests/scripts/diffusion_policy/temp/test_trajectories_rendered/data.zarr'
    data_paths = [
        'data/action',
        'data/img',
        'data/slider_state',
