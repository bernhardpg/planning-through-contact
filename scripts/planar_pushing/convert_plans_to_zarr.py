...
def main():
    ...
    log_path = data_dir.joinpath("combined_planar_pushing_logs.pkl")
    ...
    data_group.create_dataset('img', data=images, chunks=image_chunk_size)
    data_group.create_dataset('target', data=target, chunks=target_chunk_size)

    # meta_group.create_dataset('episode_ends', data=target)
