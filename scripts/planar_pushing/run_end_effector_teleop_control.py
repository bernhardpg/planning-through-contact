# This is copied from Terry Suh's po_score repo.
...
def main():
    ...
    station = builder.AddSystem(ManipulationHardwareInterface())
    station.Connect()

    robot = MultibodyPlant(1e-3)
    parser = Parser(robot)

    models_folder = Path(__file__).parents[1] / "models"
    parser.package_map().PopulateFromFolder(str(models_folder))

    directives = LoadModelDirectives(str(models_folder / "iiwa_controller_plant.yaml"))
    ProcessModelDirectives(directives, robot, parser)
    robot.Finalize()
    ...
    # ctrl_plant_context = station.GetMutableSubsystemContext(
    #    ctrl_plant, simulator.get_mutable_context()
    # )
    # ctrl_plant.SetPositions(ctrl_plant_context, q0)
