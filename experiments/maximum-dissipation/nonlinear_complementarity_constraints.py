...
def plan_planar_pushing(
    ...
    R_WBs = [np.array([[cos, sin], [-sin, cos]]) for cos, sin in zip(cos_ths, sin_ths)]
    ...
    v_c_Bs = forward_differences(
        p_c_Bs, dt
    )  # NOTE: NOT valid velocity in world frame, only valid for lam_dot really
    ...
    for k in range(num_knot_points - 1):
        ...
        # quasi_static_dynamic_constraint = eq(v_WB, f_c_B / f_max**2)
        for row in quasi_static_dynamic_constraint[0:2]:
    ...
    cos_th_vals = x_val[3 * num_knot_points : 4 * num_knot_points]
    sin_th_vals = x_val[4 * num_knot_points : 5 * num_knot_points]
    p_WB_xs_vals = x_val[5 * num_knot_points : 6 * num_knot_points]
    p_WB_ys_vals = x_val[6 * num_knot_points : 7 * num_knot_points]
    ...
    if compute_violation:
        ...
        for k in range(traj_length - 1):
            ...
            R_padded[2, 2] = 1
            R_padded[0:2, 0:2] = R
            violation = x_dot - R_padded.dot(A).dot(wrench_B)
