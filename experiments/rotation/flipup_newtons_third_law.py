...
class ContactPoint2D:
    def __init__(
        ...
        self.normal_force = sym.Variable("normal_force")
        self.friction_force = sym.Variable("friction_force")

        if "face" in contact_location:
            self.lam = sym.Variable("lam")
...
class ContactPair:
    def __init__(
        ...
        self.contact_point_A = ContactPoint2D(
            body_A, contact_location_A, friction_coeff
        )
        self.contact_point_B = ContactPoint2D(
            body_B, contact_location_B, friction_coeff
        )
...
def plan_box_flip_up_newtons_third_law():
    ...
    pc1 = ContactPair(box, "corner_4", table, "face_1", FRICTION_COEFF)
    pc2 = ContactPair(box, "face_2", finger, "corner_1", FRICTION_COEFF)
    ...
    fixed_corner = ContactPoint2D(box.nc4, box.tc4, FRICTION_COEFF)
    finger_pos = ContactPoint2D(box.n2, box.t2, FRICTION_COEFF)
